...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if _is_messages_path(self.path) and raw:
            try:
                ...
                # Model routing (off by default, PERMAFROST_MODEL_ROUTING=1 to enable)
    ...
    def _forward(self, method: str, out_bytes: bytes | None, report,
        ...
        if report is not None:  # only meter aligned /v1/messages calls
            ...
            # Quality feedback: if flash response too short → mark for upgrade
...
def main() -> None:
    ...
    # SO_REUSEPORT enables zero-downtime restart in PRoot
    try:
        httpd.socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        httpd.socket.setsockopt(socket.SOL_SOCKET, 15, 1)  # SO_REUSEPORT
    except Exception:
        pass
